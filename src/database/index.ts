...
export class DatabaseManager {
  ...
  /**
   * Find template by traversing NodeType hierarchy
   */
  private async findTemplateByNodeTypeHierarchy(nodeType: string): Promise<string | null> {
    try {
      // First, try to find a template directly on the NodeType
      const directTemplateResult = await this.session.run(`
        MATCH (nt:Node {node_type: 'NodeType', name: $nodeType})
        WHERE nt.template_id IS NOT NULL
        RETURN nt.template_id as template_id
      `, { nodeType });

      if (directTemplateResult.records.length > 0) {
        const templateId = directTemplateResult.records[0].get('template_id');
        if (templateId) {
          return templateId;
        }
      }

      // If no direct template, traverse parent NodeTypes recursively
      const parentResult = await this.session.run(`
        MATCH (child:Node {node_type: 'NodeType', name: $nodeType})-[:SUBTYPE_OF]->(parent:Node {node_type: 'NodeType'})
        RETURN parent.name as parent_name
      `, { nodeType });

      for (const record of parentResult.records) {
        const parentName = record.get('parent_name');
        if (parentName) {
          // Recursive call to check parent hierarchy
          const parentTemplate = await this.findTemplateByNodeTypeHierarchy(parentName);
          if (parentTemplate) {
      ...
      console.warn(`Error finding template for node type ${nodeType}:`, error);
      return null;
    }
  }
  ...
  async generateDocuments(nodeIdentifiers: string[], options: DocumentGenerationOptions = {}): Promise<any> {
    ...
    for (const identifier of nodeIdentifiers) {
      try {
        ...
        // Determine which template to use with hierarchical inference
        let templateId = options.template_override || nodeProps.template_id;
        
        // If no template specified, try to infer from NodeType hierarchy
        if (!templateId && nodeProps.node_type) {
          templateId = await this.findTemplateByNodeTypeHierarchy(nodeProps.node_type);
        }
        ...
        if (templateId) {
          // Try to use the specified or inferred template
          try {
